namespace FSharpLecture
module FSharpLecture =
    open System
    open ParserLibrary.ParserLibrary
    open System.Linq
    ...
    *)
    ...
    //let inline (!>) (x: obj) = match x with 
    //                           | :? bool         as v -> JBool(v)
    //                           | :? float        as n -> JNumber(n)
    //                           | :? int          as n -> JNumber(float(n))
    //                           | :? string       as s -> JString(s)
    //                           | :? seq<'K * 'T> as l -> JObject(Map.ofSeq l)
    ...
    let run(json) =
        run jValue json |> printResult

    let rec stringify(jo: JValue): string =
        match jo with
        | JString s -> sprintf "\"%s\"" (s.Replace("\"", "\\\""))
        | JNumber n -> n.ToString()
        | JBool   b -> b.ToString().ToLower()
        | JNull     -> "null"
        | JObject o -> "{" + String.Join(", ", o.Select(fun m -> (sprintf "\"%s\": %s" m.Key (stringify(m.Value))))) + "}"
        | JArray  a -> "[" + String.Join(", ", a.Select(stringify)) + "]"
